...
import { Sparkles, Loader2, Upload, X, Image as ImageIcon, ArrowLeft, Save, Plus, Wand2, FileText, Clock } from 'lucide-react';
...
export const CreatePost: React.FC = () => {
  ...
  const handleGenerateContent = async () => {
    ...
    setLoadingAI(true);
    try {
        const { content: newContent } = await GeminiService.generateBlogPost(promptTopic);
  ...
  const handleGenerateThumbnail = async () => {
    ...
    const imageBase64 = await GeminiService.generateThumbnail(imagePrompt);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
        <div>
           <div className="flex items-center text-sm text-gray-500 mb-2 flex-wrap">
              ...
              <span className="mx-2">/</span>
              <span className="text-gray-800">{isEditMode ? 'Edit' : 'Add'} Blog</span>
           </div>
           <div className="flex items-end gap-3">
             <h1 className="text-2xl font-bold text-gray-900">{isEditMode ? 'Edit Blog Post' : 'Add New Blog Post'}</h1>
        ...
        <div className="flex gap-3 w-full sm:w-auto flex-wrap">
          <button 
            ...
            className="flex-1 sm:flex-none justify-center px-4 py-2 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition"
          >
            Cancel
          </button>
          ...
          {!isEditMode && (
            <button 
              ...
              className="flex-1 sm:flex-none justify-center px-4 py-2 border border-gray-300 bg-white rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition flex items-center"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
              <div>
                ...
                <input 
                  ...
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition bg-gray-50" 
                />
              </div>
              ...
              <div>
                ...
                <textarea 
                  ...
                  rows={3}
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition bg-gray-50 resize-none" 
                />
              </div>
        ...
        <div className="space-y-6 order-1 lg:order-2">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
             <label className="block text-sm font-bold text-gray-700 mb-4">Cover Image</label>
             
             <div 
               className={`relative border-2 border-dashed rounded-xl h-48 flex flex-col items-center justify-center transition overflow-hidden bg-gray-50 ${coverImage ? 'border-indigo-500' : 'border-gray-300'}`}
             >
                    ...
                    {loadingImage ? (
                       ...
                       <>
                         ...
                         <div className="flex gap-2 mt-3 justify-center">
                           <button 
                             onClick={() => fileInputRef.current?.click()}
                             className="text-xs px-3 py-1.5 border border-gray-300 rounded-lg hover:bg-white hover:border-gray-400 transition"
                           >
                             Upload
                           </button>
                           <button 
          ...
          {/* Organization */}
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
            <h3 className="font-bold text-gray-900 mb-4 text-sm border-b border-gray-100 pb-2">Organization</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                 ...
                 <select 
                   ...
                   className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-gray-50"
                 >
                   {categories.map(cat => (
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {tags.map((tag, idx) => (
                    <span key={idx} className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-indigo-50 text-indigo-700">
                      {tag}
                ...
                <div className="relative">
                  <input 
                    ...
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-gray-50 pr-8"
                  />
              ...
              <div className="space-y-3 pt-2">
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${isPublished ? 'bg-green-500' : 'bg-gray-200'}`}
                  >
